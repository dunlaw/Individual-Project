...
func _test_prompt_formatting() -> void:
	...
	_assert("Assets available" in prompt or (LocalizationManager.get_translation("TEST_ASSET_REG_HEADER_ZH", "zh") if LocalizationManager else "") in prompt, "Should contain header")
	_assert("Generic_Lever" in prompt, "Should include asset IDs from the list")
	...
	_assert((LocalizationManager.get_translation("TEST_ASSET_REG_HEADER_ZH", "zh") if LocalizationManager else "") in prompt, "Should include Chinese text")
	_assert("Assets available" in prompt, "Should include English text")

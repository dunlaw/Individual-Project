...
const TEST_ROOTS: Array[String] = [
	"res://1.Codebase/Unit Test",
	"res://1.Codebase/src/scripts/tests",
	"res://1.Codebase/src/scripts/ui",
]
...
func _run_all_tests() -> void:
	...
	for root in TEST_ROOTS:
		discovered.append_array(_discover_test_files(root))
	discovered.sort()
	var total_suites := 1 + discovered.size()   
	print("\n   Discovered %d test files  (+1 inline suite)  = %d suites total" \
			% [discovered.size(), total_suites])
	print("   Discovery roots: %s" % ", ".join(TEST_ROOTS))
...
func _read_results(inst: Node) -> Dictionary:
	...
	if "_pass_count" in inst and "_fail_count" in inst:
		return {
			"passed": inst.get("_pass_count"),
			"failed": inst.get("_fail_count"),
		}

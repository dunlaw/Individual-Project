...
func send_request(messages: Array, callback: Callable, _options: Dictionary = {}) -> void:
	if not is_configured():
		_emit_error("OpenAI API key is not configured")
		_notify_callback_failure(callback, "OpenAI API key is not configured")
		return
	...
	_store_debug_request("openai", OPENAI_ENDPOINT, json_body, { "model": model })
	_emit_progress({"status": "sending", "body_bytes": json_body.length()})
	var error = http_request.request(OPENAI_ENDPOINT, headers, HTTPClient.METHOD_POST, json_body)
	if error != OK:
		is_requesting = false
		var err_msg = "Failed to send OpenAI request: " + str(error)
		_emit_error(err_msg)

...
var use_auto_router: bool = false
...
func get_configuration() -> Dictionary:
	return {
		"api_key": api_key,
		"model": model,
		"use_auto_router": use_auto_router,
	}
func apply_configuration(config: Dictionary) -> void:
	...
	if config.has("use_auto_router"):
		use_auto_router = bool(config["use_auto_router"])
func send_request(messages: Array, callback: Callable, _options: Dictionary = { }) -> void:
	...
	var effective_model := OPENROUTER_AUTO_MODEL if use_auto_router else model

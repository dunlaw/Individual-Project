...
## Static helpers for building the SettingsMenu tab layout.
##
## All functions are stateless.  The main rebuild entry point is
## `rebuild_tabs()`, which accepts node references via a Dictionary,
## builds the TabContainer hierarchy, and returns the newly-created
## nodes for the caller to assign back to its own variables.

# ── Small node-manipulation utilities ────────────────────────────────────────
...
## Builds and populates the full tab layout.
##
## Parameters:
##   main_vbox              – root VBoxContainer of the settings panel
##   original_scroll        – the original non-tabbed scroll (will be hidden)
##   nodes                  – Dictionary of @onready node refs keyed by name
##   on_gloria_voice_toggled – Callable for the gloria-voice CheckBox signal
##   create_ai_log_page_fn  – Callable() -> VBoxContainer for the AI log tab
##
## Returns a Dictionary with all newly-created nodes that the caller must
## assign back to its own variables:
##   tab_container, tab_gameplay, tab_display, tab_audio, tab_voice,
##   tab_tutorial, tab_developer,
##   text_speed_label, text_speed_option, screen_shake_check,
##   max_rounds_label, max_rounds_spinbox, gloria_voice_check

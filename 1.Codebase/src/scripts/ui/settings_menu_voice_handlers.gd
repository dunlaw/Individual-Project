...
## Manages voice state and all voice-tab event handlers for SettingsMenu.
##
## Usage (two-step initialisation matching the developer-handlers pattern):
##   1. _voice_ctrl = SettingsMenuVoiceHandlers.new()
##      _voice_ctrl.setup(get_ai_manager_fn, get_audio_manager_fn,
##                        apply_audio_settings_fn, play_sfx_fn, tr_fn,
##                        set_button_safely_fn)
##   2. After _ready() resolves @onready vars, pass node refs and initialise:
##      _voice_ctrl.set_node_refs({ "availability_label": ..., ... })
##      _voice_ctrl.initialize()
...
# ── Service callables (set in setup) ──────────────────────────────────────────
var _get_ai_manager_fn: Callable       # () -> Node|null
var _get_audio_manager_fn: Callable    # () -> Node|null
var _apply_audio_settings_fn: Callable # () -> void
var _play_sfx_fn: Callable             # (name: String) -> void
var _tr_fn: Callable                   # (key: String) -> String
var _set_button_safely_fn: Callable    # (button: BaseButton, pressed: bool) -> void

# ── UI node refs (set in set_node_refs) ───────────────────────────────────────
...
# ── Step 1 – store service callables ──────────────────────────────────────────
func setup(
...
# ── Step 2 – receive UI node refs after @onready resolves ─────────────────────
...
# ── Step 3 – populate dropdowns, load settings, connect AIManager signals ─────
## preloaded: optional dict from load_settings() with persisted voice state,
## used as fallback defaults before querying AIManager.
...
# ── UI event handlers (called from thin wrappers in settings_menu.gd) ─────────

...
The context-management system operates at two complementary layers: \textbf{storage-level compression} in \texttt{AIMemoryStore} and \textbf{runtime budget-aware assembly} in \texttt{AIPromptBuilder} with \texttt{AIContextDelta}. Together they ensure the final prompt stays within the token budget while preserving narrative coherence.

\subsubsection{Storage Layer --- AIMemoryStore}

I implemented three distinct data structures within the \texttt{AIMemoryStore} class (\texttt{ai\_memory\_store.gd}). The short-term memory layer retains the most recent entries calculated as \texttt{max(SHORT\_TERM\_WINDOW, memory\_full\_entries * 2)}; with \texttt{SHORT\_TERM\_WINDOW = 5} and \texttt{memory\_full\_entries = 6}, this yields \texttt{max(5, 12) = 12} entries kept in full detail. When story memory exceeds the configurable threshold of twenty-four entries, older entries are migrated to the long-term summaries layer. Crucially, this migration is \textbf{sampling-based compression rather than semantic summarisation}: the \texttt{\_summarize\_entries()} function selects representative samples at the earliest, quarter, middle, three-quarter, and latest positions from the archived block, tagging each with its temporal position (e.g.\ ``Earliest'', ``Early'', ``Middle'', ``Late'', ``Latest summary''). For a block of one hundred entries, five representative samples are retained. This approach preserves key narrative landmarks without requiring an additional LLM call for summarisation. The notes register layer maintains persistent facts and constraints that should influence all generations, regardless of when they were established.

\subsubsection{Runtime Layer --- AIPromptBuilder and AIContextDelta}

The true runtime compression occurs in \texttt{AIPromptBuilder} (\texttt{ai\_prompt\_builder.gd}) working with \texttt{AIContextDelta} (\texttt{ai\_context\_delta.gd}). Rather than naively concatenating all context, the builder operates within a token budget (default 24,000 tokens, estimated at roughly four characters per token) and performs \textbf{incremental, budget-aware prompt assembly}:

\begin{enumerate}
    \item \textbf{Token reservation}: Before assembling context sections, the builder estimates the minimum user-message size and reserves that many tokens from the budget, ensuring the player's prompt is never crowded out by context sections.
    \item \textbf{Section fingerprinting}: Each context section (static rules, system persona, entropy modifiers, long-term summaries, notes, short-term memory) is fingerprinted by hashing its content. \texttt{AIContextDelta} compares each section's current fingerprint against the previous request's fingerprint.
    \item \textbf{Incremental sending}: If a section is unchanged from the previous request, the builder substitutes a lightweight marker --- \texttt{[context:\{section\_name\} unchanged from previous request]} --- saving the tokens that would have been spent resending identical content. Sections are forced to resend after eight consecutive unchanged cycles to guard against context drift.
    \item \textbf{Budget overflow degradation}: When a changed section exceeds the remaining budget, the builder replaces it with a compressed summary marker --- \texttt{[context:\{section\_name\} updated, \{msg\_count\} messages, \~{}\{total\_chars\} chars --- truncated for budget]} --- preserving awareness of the section's existence without transmitting its full content.
    \item \textbf{Short-term memory trimming}: Short-term entries are appended one by one until the budget is exhausted; any remaining entries are summarised as omitted counts.
    \item \textbf{Final user message truncation}: If the user message itself exceeds the remaining budget after all context has been assembled, it is truncated to fit.
\end{enumerate}
...
The \textbf{AI Usage Analytics Export} (\texttt{SettingsMenuAILogExport}) emerged directly from my evaluation methodology. To perform the functional testing described in Section 9, I needed structured records of every AI request, not just pass/fail observations. I implemented a telemetry pipeline in which \texttt{AIRequestManager} captures a dictionary of metadata for each API call, including provider, model, token counts, latency, mode, purpose, prompt text, and response text. These records accumulate in memory and are persisted via \texttt{AIUsageStatsStore} to the user data directory. The Settings menu then exposes CSV and JSON export buttons: the CSV export additionally invokes \texttt{SettingsMenuAIAnalytics} to compute derived series (provider success rates, hourly call distributions, cumulative token curves, tokens-per-second throughput) before writing the file. The CSV produced during testing is included in the project repository as a concrete evaluation artefact, demonstrating actual provider performance across the functional testing period.
...
I developed over sixty unit test scripts located in the \texttt{Unit Test/} directory and additional test directories, covering core systems including \texttt{test\_ai\_prompt\_builder.gd}, \texttt{test\_event\_bus.gd}, \texttt{test\_game\_state.gd}, \texttt{test\_save\_load\_system.gd}, and \texttt{test\_butterfly\_effect\_tracker.gd}. These tests verify that deterministic components behave correctly given specific inputs, enabling confident refactoring without fear of introducing regressions.
...
%% Reduce appendix text size and spacing to stay within ~20 pages total
\small
\singlespacing
...
\section{Ethics Compliance Form}
...
\begin{center}
{\large\textbf{G5038: Final Year Project --- Week 3 Project Proposal}}\\[0.3cm]
{\large\textbf{Title: AI-Powered Dynamic Narrative System for Role-Playing Games (RPGs)}}\\[0.3cm]
\textbf{Candidate Number:} 281967 \quad \textbf{Supervisor:} Dan Creed \quad \textbf{Date:} 09 October 2025\\[0.3cm]
...
\begin{center}
{\large\textbf{G5038: Final Year Project --- Game Design Document}}\\[0.3cm]
{\large\textbf{Game Title: Glorious Deliverance Agency 1}}\\[0.3cm]
University of Sussex, Engineering \& Informatics\\[0.3cm]
...
\begin{itemize}
  ...
  \item Sixty-nine character sprites: 48 expression sprites across six characters (eight each), 6 full-body turnaround sheets, 6 portrait busts, and 9 additional specialty performance poses for Teacher Chan
...
\begin{figure}[H]
  \centering
  \includegraphics[height=5cm]{../1.Codebase/src/assets/characters/turnaround_protagonist.png}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.18\linewidth}\centering
    \includegraphics[height=4cm]{../1.Codebase/src/assets/characters/turnaround_gloria.png}
    \caption*{\textbf{Saint Gloria}}\end{subfigure}\hfill
  \begin{subfigure}[b]{0.18\linewidth}\centering
    \includegraphics[height=4cm]{../1.Codebase/src/assets/characters/turnaround_donkey.png}
    \caption*{\textbf{Sir Donkey}}\end{subfigure}\hfill
  \begin{subfigure}[b]{0.18\linewidth}\centering
    \includegraphics[height=4cm]{../1.Codebase/src/assets/characters/turnaround_ark.png}
    \caption*{\textbf{ARK}}\end{subfigure}\hfill
  \begin{subfigure}[b]{0.18\linewidth}\centering
    \includegraphics[height=4cm]{../1.Codebase/src/assets/characters/turnaround_one.png}
    \caption*{\textbf{One}}\end{subfigure}\hfill
  \begin{subfigure}[b]{0.18\linewidth}\centering
    \includegraphics[height=4cm]{../1.Codebase/src/assets/characters/turnaround_teacher_chan.png}
    \caption*{\textbf{Teacher Chan}}\end{subfigure}
  \caption{NPC full-body character designs --- \textit{Glorious Deliverance Agency 1} (pixel art)}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\linewidth]{../1.Codebase/src/assets/ui/praytoMonster.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\linewidth]{../1.Codebase/src/assets/ui/miss_chan_on_stage.png}
...
\begin{itemize}
  ...
  \item Sixty-nine character sprites: 48 expression sprites (8 per character across 5 characters), 16 specialty performance poses for Teacher Chan, 6 full-body turnaround sheets, and 6 portrait busts
  ...
  \item 42 audio assets: 12 background music tracks (entropy-scaled by narrative stage) and 30 dedicated sound effects
\end{itemize}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\linewidth]{../1.Codebase/src/assets/ui/butterfly_effect.png}
...
The Achievement System has been fully implemented with exactly 20 named achievements and one hidden achievement (\texttt{one\_s\_voice}), each backed by its own icon in \texttt{src/assets/achievements/}. The system serves three purposes:
\begin{enumerate}
...
The \texttt{TeammateSystem} has been fully implemented with a structured \texttt{BEHAVIOR\_LIBRARY} containing 12 named behaviours and a \texttt{TEAMMATES} dictionary defining all four NPCs. Each behaviour entry specifies a label, summary, interaction style, and direct stat impact \texttt{\{reality, positive, entropy\}}. The 12 behaviours are:
...
\begin{itemize}
  ...
  \item \textbf{Donkey's repertoire:} \texttt{heroic\_nonsense} ($-5/+6/+4$), \texttt{blame\_shifting} ($-7/+5/+2$), \texttt{epic\_planning} ($-3/+7/+3$)
  \item \textbf{ARK's repertoire:} \texttt{overcomplicate} ($-6/+4/+5$), \texttt{black\_box\_operation} ($-5/+6/+6$), \texttt{absolute\_plan} ($-4/+5/+7$)
...
Prompts are assembled by the \texttt{NarrativePromptBuilder} and \texttt{AIPromptBuilder} from structured templates managed by the \texttt{SkillManager}. The SkillManager scans the \texttt{src/skills/} directory (15 skill folders at time of submission) and parses each \texttt{SKILL.md} file's YAML frontmatter for metadata including \texttt{purpose\_triggers}. Skill folders include: \texttt{mission-generation}, \texttt{teammate-interference}, \texttt{gloria-intervention}, \texttt{prayer-system}, \texttt{trolley-problem}, \texttt{scene-directives}, \texttt{character-profiles}, \texttt{choice-followup}, \texttt{consequence-generation}, \texttt{entropy-effects}, \texttt{game-recap}, \texttt{honeymoon-phase}, \texttt{intro-story}, \texttt{night-cycle}, and \texttt{force-mission-complete}. This externalises all prompt engineering from GDScript, enabling prompt iteration without recompilation.
...
The FSM 30-Day Rebirth Challenge is a fully implemented standalone game mode, accessible from the main menu, that reframes the core loop as a structured 30-day self-improvement programme endorsed by the Flying Spaghetti Monster. Players commit to completing one daily mission for 30 consecutive days, each day presenting a themed content card (backed by per-day pixel-art assets in \texttt{src/assets/rebirth\_challenge/}, days 1--8 illustrated). The mode is managed by the \texttt{FSMChallengeModule} class, which tracks \texttt{challenge\_start\_date}, \texttt{current\_day}, \texttt{days\_completed}, and \texttt{last\_login\_date} for continuity enforcement.
...
\begin{itemize}
  \item \textbf{Chopin --- Nocturne in E-flat major, Op.\,9 No.\,2} (\texttt{chopin\_nocturne\_op9\_no2.mp3}). Frédéric Chopin (1810--1849). Published 1833. Public domain.
...
The game implements a comprehensive AI usage telemetry system designed to support both research evaluation and provider cost management. Every API call made during gameplay is recorded by the \texttt{AIRequestManager}, which captures: timestamp, provider name, model identifier, HTTP status code, input and output token counts, wall-clock response latency, request mode (\texttt{live}/\texttt{mock}/\texttt{mock\_fallback}), request purpose (e.g.\ \texttt{mission-generation}, \texttt{prayer-system}), any error message, the full serialised request body (including assembled prompt), and the raw response body. These records are persisted to a JSON file in Godot's sandboxed user data directory by the \texttt{AIUsageStatsStore} class.

The Settings menu exposes two export actions through \texttt{SettingsMenuAILogExport}:

\begin{itemize}
  \item \textbf{CSV Export} (file: \texttt{ai\_usage\_charts\_[timestamp].csv}): produces a two-section CSV. The first section contains one row per API call with columns \texttt{section}, \texttt{timestamp}, \texttt{provider}, \texttt{model}, \texttt{status}, \texttt{input\_tokens}, \texttt{output\_tokens}, \texttt{response\_time\_sec}, \texttt{mode}, \texttt{purpose}, \texttt{error}, \texttt{prompt\_text}, \texttt{prompt\_modules\_active}, and \texttt{ai\_response\_text}. The second section appends derived analytics series computed by \texttt{SettingsMenuAIAnalytics}, including: provider success rates, total and per-provider token usage, average response times, tokens-per-second throughput, mode distribution (live/mock/fallback), model call counts, hourly call and token breakdowns, and cumulative token progression. This format is designed for direct import into spreadsheet tools for visual analysis.
  \item \textbf{JSON Export} (file: \texttt{ai\_call\_log\_[timestamp].json}): exports the complete raw call log alongside a pre-computed metrics summary dictionary, suitable for programmatic analysis.
\end{itemize}

These export capabilities serve the project's research methodology by creating auditable records of AI system behaviour during functional testing sessions. The CSV produced during development is available as an artefact in the project root (\texttt{ai\_usage\_2026-03-30\_19-38-42.csv}), demonstrating actual provider performance data collected across the functional testing period.
...
An extensive testing infrastructure has been implemented, comprising 20+ automated test files located in \texttt{/5.Codebase/Unit Test/}, providing comprehensive coverage across all major system components. The testing strategy employs Godot's built-in testing framework (GUT --- Godot Unit Test) with assertion-based validation.
...
\begin{itemize}
  \item \texttt{test\_game\_state.gd} (state management, game phase transitions)
  \item \texttt{test\_player\_stats.gd} (stat calculation formulas, including void entropy formula verification, skill checks with 1d10 mechanics, boundary conditions for 0--100 ranges)
  \item \texttt{test\_gamestate\_integration.gd} (save/load persistence, JSON serialisation integrity)
  \item \texttt{test\_event\_bus.gd} (event system, signal propagation across decoupled components)
\end{itemize}

\textbf{AI System Tests:}
\begin{itemize}
  \item \texttt{test\_ai\_prompt\_builder.gd} (context construction, note filtering by importance and tags, prompt layering verification)
  \item \texttt{test\_ai\_system.gd} (provider switching between Gemini/Ollama/Mock, fallback behaviour, timeout handling)
\end{itemize}

Testing methodology employs assertion-based validation for expected vs.\ actual outcomes (\texttt{assert\_eq}, \texttt{assert\_true}, \texttt{assert\_almost\_eq} for floating-point comparisons), edge case coverage for boundary conditions (reality score at 0/100, entropy at 0.0/1.0), empty inputs, and error states, and mock data injection for deterministic testing without network dependencies.
...
\textbf{UI and Logic Flow:} Complete player interaction loops tested using \texttt{test\_story\_scene\_modules.gd} for choice selection, stat updates via GameState delegation to PlayerStats, and UI refresh after state changes. Special event flows (Prayer System, Gloria Intervention, Moral Dilemmas) were tested with both MockAIGenerator for deterministic outcomes and live API calls for robustness.

\textbf{AI Context and Memory Integration:} \texttt{test\_ai\_prompt\_builder.gd} verifies that AIManager receives accurate game state context (reality score, positive energy, entropy) and includes filtered notes (importance $\geq 3$) and butterfly effect data. Thematic appropriateness is tested by validating that prompt construction modifies tone based on entropy thresholds (low/medium/high).
...
\textbf{Performance Monitoring:} Tests log AI latency and token consumption during test runs. Observed metrics align with expectations: local Ollama responses 2--10 seconds depending on model size, cloud API (Gemini) responses 1--5 seconds, and token consumption 500--1500 tokens per narrative generation request.

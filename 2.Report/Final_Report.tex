...
The memory system maintains three tiers: short-term verbatim history (most recent 5--12 entries), long-term algorithmic summaries (up to 16 compressed batches), and persistent importance-scored notes (up to 32 entries). Compression from short-term to long-term is triggered automatically when the rolling log exceeds a configurable threshold, using a local sampling algorithm that requires no additional AI call. At the prompt-assembly layer, a three-layer length-control mechanism (incremental delta fingerprinting, budget-aware section degradation, and hard character truncation) prevents token budget overrun whilst preserving as much narrative context as possible. Full technical specifications --- including all parameters, the eight-section prompt structure, and the three-layer control tables --- are documented in Appendix C: GDD, Sections 7.1--7.2.
...
Each AI request is assembled by \texttt{build\_prompt()} in eight prioritised sections, from static world context through to the player's current message, with lower-priority sections gracefully degraded or replaced by lightweight markers when the token budget is exhausted. The \texttt{max\_tokens} ceiling also directly affects narrative completeness: too low a value causes truncated directives and missing choice options; the design target of 4096 tokens comfortably accommodates a full narrative response. The complete section structure and output-length analysis are documented in Appendix C: GDD, Section 7.2.
...
Context window management presented a genuine engineering challenge. Simply including entire conversation history led to both token cost issues and degraded output quality. The final implementation therefore combines stored continuity summaries with runtime budget-aware assembly rather than replaying full transcripts.

The three-tier memory architecture (short-term history, long-term algorithmic summaries, and persistent notes) is assembled through a token-budget-aware pipeline that applies three layers of length control: incremental fingerprint delta to avoid re-sending unchanged sections, budget-aware section degradation from full content to summary to marker, and hard character truncation for the player's own prompt text. Story history is automatically compressed by a local sampling algorithm once the rolling log exceeds a configurable threshold, without requiring an additional AI request. Complete specifications including all parameters, pseudocode, and control tables are in Appendix C: GDD, Section 7.2.
...
%% ============ GDD SECTION 7: TECHNICAL ARCHITECTURE ============
\subsection*{7. Technical Architecture Specifications}

\subsubsection*{7.1 AI Provider Architecture}
The system uses the Strategy design pattern to support seven concrete AI backends plus a built-in mock mode. At runtime, \texttt{AIProviderManager.get\_current\_provider()} reads the active provider from \texttt{AIConfigManager} and returns the appropriate provider instance:

\begin{itemize}
  \item \textbf{Google Gemini} --- primary cloud provider; supports JSON schema enforcement and Gemini Live voice streaming via \texttt{VoiceBridge}.
  \item \textbf{OpenRouter} --- multi-model routing gateway; forwards requests to third-party models without per-model integration work.
  \item \textbf{Ollama} --- local inference; used for unlimited offline development and testing without API cost.
  \item \textbf{OpenAI, Claude, LM Studio} --- alternative cloud providers with identical interface contracts.
  \item \textbf{AI Router} --- automatic load-balancing across multiple providers based on latency and availability.
  \item \textbf{Mock Generator} --- deterministic, cost-free responses driven by \texttt{context["purpose"]}; used for UI and logic testing. Note: mock mode validation skips both min-length and max-length checks.
\end{itemize}

Provider selection is persisted in \texttt{GameState} and exposed through the in-game AI Settings menu. The facade pattern in \texttt{AIManager} means all calling code is provider-agnostic: generation methods accept context dictionaries and return structured response objects regardless of the active backend.

\subsubsection*{7.2 Context Management and Token Budget Control}

\paragraph{Memory Tier Architecture.}
The context-management system maintains three independent tiers of narrative state, each with a distinct retention policy and injection point within the assembled prompt:

\begin{enumerate}
  \item \textbf{Short-term memory} (\texttt{story\_memory} tail, 5--12 entries) --- The most recent story exchanges, injected verbatim as prompt Section 7. These provide the AI with immediate conversational continuity. Entries are held until the rolling log exceeds \texttt{max\_memory\_items} (hard cap: 120).
  \item \textbf{Long-term summaries} (\texttt{long\_term\_summaries}, up to 16 entries) --- Algorithmically compressed representations of older history, injected as prompt Section 5. Generated locally by \texttt{\_update\_long\_term\_memory()} without any additional AI call.
  \item \textbf{Persistent notes} (\texttt{notes\_register}, up to 32 entries, prioritised by importance score 1--5) --- Structured facts, character relationships, and narrative constraints, injected as prompt Section 6 (up to \texttt{MAX\_NOTES\_PER\_PROMPT = 8} per request). Higher-importance notes are retained when the register reaches capacity.
\end{enumerate}

\paragraph{Eight-Section Prompt Assembly.}
\texttt{AIPromptBuilder.build\_prompt()} assembles each AI request from eight sections inserted in strict priority order. Sections are added against a running token budget; any section that cannot fit in full is either summarised or replaced by a lightweight marker (see the three-layer mechanism below).
...
7 & \texttt{short\_term\_memory} & Most recent conversation exchanges (5--12) \\
8 & \texttt{user\_message}       & Current player input and session metadata \\
\hline
\end{tabular}
\caption*{Table C.1 — Eight sections assembled by \texttt{build\_prompt()} in priority order.}
\end{table}

This ordering guarantees that the most critical control information (system persona, entropy-based tone modifier) is always transmitted even when the token budget is severely constrained.

\paragraph{Output Length and Gameplay Completeness.}
The \texttt{max\_tokens} parameter passed to each provider directly determines whether an AI response is narratively complete. At the design target of 4096 tokens, a single response comfortably contains a full narrative paragraph, all three player-choice options, and a complete \texttt{[SCENE\_DIRECTIVE]} JSON block. If the limit is set below approximately 500 tokens, the AI may truncate sentences mid-clause, produce malformed directives, or emit fewer than three choice options, causing the UI parser to fail. Raising the limit beyond 4096 (e.g.\ 8192) rarely increases observable content because the model naturally concludes earlier, but it does increase per-request API cost. The 4096-token default is therefore a deliberate balance between content completeness and operational cost.

\paragraph{Three-Layer Prompt Length Control.}
To prevent any single request from exceeding its token budget, three independent mechanisms apply in sequence:

\textbf{Layer 1 — Incremental Delta Fingerprinting (\texttt{AIContextDelta}).}
Before insertion, each section is hashed to a fingerprint. If the hash matches the previous request's recorded value, the full section content is replaced by a single-line marker:
\begin{verbatim}
[context:long_term_context unchanged from previous request]
\end{verbatim}
This saves 60--80\% of static-context token usage across a multi-round session. A staleness counter forces full re-transmission after eight consecutive unchanged rounds to prevent the model from ``forgetting'' long-standing context.

\textbf{Layer 2 — Budget-Aware Section Degradation (\texttt{\_append\_user\_context\_block}).}
When a section has changed and must be transmitted, the builder tries three strategies in sequence: (i) full content if it fits within the remaining budget, (ii) a one-line algorithmic summary if the full content is too large, and (iii) an unchanged marker if even the summary cannot fit. This ensures the model always receives at least an indication that a section exists, even under extreme budget pressure.

\textbf{Layer 3 — Hard Character Truncation (\texttt{\_build\_prompt\_chunk}).}
As a last resort, the player's prompt text is truncated to the exact character count corresponding to the remaining token allocation (\texttt{available\_tokens $\times$ CHARS\_PER\_TOKEN}, where \texttt{CHARS\_PER\_TOKEN = 4}), and a \texttt{[prompt truncated for budget]} notice is appended so the model is aware that the input was cut short.
...
\hline
\end{tabular}
\caption*{Table C.2 — Three-layer prompt length control applied during each \texttt{build\_prompt()} call.}
\end{table}

\paragraph{Automatic Story Compression (Local Algorithm).}
When \texttt{story\_memory} exceeds \texttt{memory\_summary\_threshold} (default: 24 entries), \texttt{AIMemoryStore} calls \texttt{\_update\_long\_term\_memory()}, which compresses the oldest entries using a local sampling algorithm — no additional AI request is made. The algorithm selects representative entries at five positions (first, quarter-point, midpoint, three-quarter-point, last) and formats them as a timestamped narrative timeline. The summary is appended to \texttt{long\_term\_summaries} and the sampled entries are removed from the rolling log.

This local-only approach eliminates compression latency, produces deterministic and serialisable outputs, and avoids the risk of an AI summariser over-interpreting narrative history.
...
\hline
\end{tabular}
\caption*{Table C.3 — Memory management parameters in \texttt{AIMemoryStore}.}
\end{table}

\subsubsection*{7.3 Response Parsing and Canonicalisation}
\texttt{SceneDirectivesParser} implements multi-stage extraction: it first searches for explicit \texttt{[SCENE\_DIRECTIVES]} markers, then falls back to JSON extraction from markdown code blocks, and finally attempts to parse the full response as raw JSON. All extracted background and asset identifiers are canonicalised (e.g.\ ``Crystal Cavern'' $\rightarrow$ ``crystal\_cavern'') to match the internal asset catalogue before the directive is applied.

\subsubsection*{7.4 Safety and Content Filtering}
\texttt{AISafetyFilter} applies two passes on both user input and model output. The first pass scrubs sensitive data patterns (API keys, email addresses, credit card numbers) using regex. The second pass matches against keyword groups for harmful content categories (self-harm, violence, hate speech). Blocked responses are replaced with a generic safe message rather than silently discarded.

\subsubsection*{7.5 Rate Limiting and Error Handling}
\texttt{AIRequestRateLimiter} enforces configurable minimum intervals between API calls system-wide (not per-provider) to proactively prevent HTTP 429 errors. Every AI request includes a 24-second timeout; on expiry the game returns to a stable state rather than freezing. \texttt{ErrorReporter} provides centralised severity-tagged logging (warning vs.\ error) to distinguish recoverable failures from critical ones.

\subsubsection*{7.6 Analytics and Telemetry}
\texttt{AIRequestManager} records one telemetry entry per API call (provider, model, latency, token usage, mode, purpose, prompt, response). \texttt{AIUsageStatsStore} persists the log to \texttt{user://ai\_call\_log.json}. The Settings menu exposes JSON and CSV export via \texttt{SettingsMenuAILogExport}, and in-game charts rendered by \texttt{AIChartCanvas} summarise provider performance and token-usage distribution across a session.
